...
class TemplateRegistry(object):
    ...
    def submit_share(self, job_id, worker_name, session, extranonce1_bin, extranonce2, time, nonce, difficulty):
        '''Check parameters and finalize block template. If it leads
           to valid block candidate, asynchronously submits the block
           back to the bitcoin network.
        
            - extranonce1_bin is binary. No checks performed, it should be from session data
            - job_id, extranonce2, time, nonce - in hex form sent by the client
	...
	if settings.COINDAEMON_ALGO == 'heavy':
        	coinbase_hash = securitycoin_hash.getHash(coinbase_bin, len(coinbase_bin))
	else:
        ...
        if settings.COINDAEMON_ALGO == 'scrypt':
            ...
            hash_bin = securitycoin_hash.getPoWHash(header_bin)
            # hash_bin = securitycoin_hash.getPoWHash(''.join([ header_bin[i*4:i*4+4][::-1] for i in range(0, 21) ]))
	elif settings.COINDAEMON_ALGO == 'skeinhash':
        ...
        if hash_int <= job.target:
            ...
            if settings.COINDAEMON_ALGO == 'heavy':
                block_hash_bin = securitycoin_hash.getHash(header_bin, 80)
            else:
