...
if settings.COINDAEMON_ALGO == 'scrypt':
    ...
    log.debug("########################################### Loading Heavycoin Support #########################################################")
...
class CTransaction(object):
    ...
    def calc_heavy(self):
        if self.heavy is None:
            self.heavy = uint256_from_str(securitycoin_hash.getHash(self.serialize(), len(self.serialize())))
        return self.heavy
...
class CBlock(object):
       ...
       def calc_scrypt(self):
         ...
         def calc_heavy(self):
             if self.heavy is None:
                ...
                r.append(struct.pack("<I", self.nBits))
                r.append(struct.pack("<I", self.nNonce))
                self.heavy = uint256_from_str(securitycoin_hash.getPoWHash(''.join(r)))
             return self.heavy
    else:
       def calc_sha256(self):
           if self.sha256 is None:
    ...
    def is_valid(self):
        ...
        for tx in self.vtx:
            ...
            if settings.COINDAEMON_ALGO == 'heavy':
                tx.calc_heavy()
                hashes.append(ser_uint256(tx.heavy))
            else:
                tx.calc_sha256()
        ...
        while len(hashes) > 1:
            ...
            for i in xrange(0, len(hashes), 2):
                ...
                if settings.COINDAEMON_ALGO == 'heavy':
                    newhashes.append(securitycoin_hash.getHash(self.serialize(), len(self.serialize())))
                else:
