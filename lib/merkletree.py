...
class MerkleTree:
        ...
        if detailed or Ll > 1:
            while True:
                ...
                if Ll % 2:
                    L += [L[-1]]
		if settings.COINDAEMON_ALGO == 'heavy':
                    L = PreL + [securitycoin_hash.getHash(L[i] + L[i + 1], len(L[i] + L[i + 1])) for i in range(StartL, Ll, 2)]
		else:
    ...
    def hash_steps(self):
        if self._hash_steps == None:
            if settings.COINDAEMON_ALGO == 'heavy':
	        self._hash_steps = securitycoin_hash.getHash(''.join(self._steps), len(''.join(self._steps)))
            else:
    ...
    def withFirst(self, f):
        ...
        for s in steps:
            if settings.COINDAEMON_ALGO == 'heavy':
                f = securitycoin_hash.getHash(f + s, len(f + s))
            else:
                f = doublesha(f + s)
        return f
